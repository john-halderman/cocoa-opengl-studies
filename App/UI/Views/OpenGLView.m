...
//	NSLog(@"================> %f, %f, %f", self.timeValue, self.timeValue - self.lastTimeValue, self.averageDrawInterval);
	...
	Vector2 p = { .x = 0.5f * sinf(self.timeValue), .y = 0.5f * cosf(self.timeValue) };
//	NSLog(@"================> %.5f, %.5f", p.x, p.y);
	glUniform2fv(self.positionUniform, 1, (const GLfloat *)&p);
	printGlErrors(@"glUniform2fv(positionUniform)");

	if(self.noiseTime > 0 ) {
		glUniform1f( self.noiseTime, self.timeValue );
...
- (void)loadBufferData {
    Vertex vertexData[4] = {
        { .position = { .x=-0.5, .y=-0.5, .z=0.0, .w=1.0 }, .color = { .r=1.0, .g=0.0, .b=0.0, .a=1.0 } },
        { .position = { .x=-0.5, .y= 0.5, .z=0.0, .w=1.0 }, .color = { .r=0.0, .g=1.0, .b=0.0, .a=1.0 } },
        { .position = { .x= 0.5, .y= 0.5, .z=0.0, .w=1.0 }, .color = { .r=0.0, .g=0.0, .b=1.0, .a=1.0 } },
        { .position = { .x= 0.5, .y=-0.5, .z=0.0, .w=1.0 }, .color = { .r=1.0, .g=1.0, .b=1.0, .a=1.0 } }
    };
    ...
    glVertexAttribPointer((GLuint)self.colorAttribute  , 4, GL_FLOAT, GL_FALSE, sizeof(Vertex), (const GLvoid *)offsetof(Vertex, color  ));
